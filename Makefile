...
datavolumes:
	docker run -d -v $(RDBDATADB):/data/$(NODENAME_RDB)_datadb --name $(RDBDATADB) --entrypoint /bin/echo debian:wheezy
	docker run -d -v $(RDBDATA):/data --name $(RDBDATA) --entrypoint /bin/echo debian:wheezy

backup:
	docker run --rm --volumes-from $(RDBDATADB) -v $(PWD):/backup debian:wheezy bash -c "tar zcvf /backup/$(RDBDATADB).tgz data/$(NODENAME_RDB)datadb"
...
rethinkdb:
	docker run -d --net icec --ip $(SUBNET).12 -e SUBNET=$(SUBNET) --volumes-from=$(RDBDATADB) -p 8080:8080 --name $(NODENAME_RDB) rethinkdb:latest rethinkdb

rmrethinkdb:
	docker rm -f $(NODENAME_RDB)

rmdatavolumes:
	docker rm -f $(RDBDATADB)
	docker volume rm $(RDBDATADB)
	docker rm -f $(RDBDATA)
